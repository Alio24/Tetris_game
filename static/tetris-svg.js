...
window.addEventListener("load", function (ev) {
    ...
    var tryFall = function () {
        var next = block.fall();
        if (next.ok(stage)) {
            // If the block can still fall, update its position and render the game.
            block = next;
            return render();
        } else {
            // If the block cannot fall, place it on the stage and continue.
            block.put(stage);
            render();
            
            // After a short delay, shrink the stage, create a new block, and trigger AI for its placement.
            setTimeout(function () {
                stage.shrink();
                block = newBlock();
                trigger_ai(block.moves, block.rotations);
    
                // If the new block cannot be placed, prompt a message and restart the game if confirmed.
                if (!block.ok(stage)) {
